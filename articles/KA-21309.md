...
<b>准备</b>
  步骤1. 数据清理步骤2. 重新启动时间步骤3。 每小时访问次数步骤4。 最大并发处理步骤5。 拆分日志文件步骤6。 合并请求记录和响应记录

<b>分析示例</b>
  示例1. 访问次数最多的示例2。 访问缺少的响应示例3。 访问速度慢示例4。 响应时间的时间序列数据示例5。 最小、平均（平均）、中值、最大响应时间示例6。 每个时段的访问次数示例7。 每个时段的响应状态数示例8。 最常使用的URL示例9。 request.log记录的access.log记录

<b>结论</b>

<br>简介<br><br>request.log的格式<br><br>
...
<b>请求记录</b>
  当AEM收到请求时，将记录请求记录。 它包含接收日期和时间、请求ID、请求方法和URL。

<b>响应记录</b>
  当AEM响应请求时，将记录响应记录。 它包含响应日期和时间、请求ID、状态代码、Content-Type和响应时间（以毫秒为单位）。
...
<br>示例5. 最小、平均（平均）、中值、最大响应时间<br><br>
...
<br>示例9. request.log记录的access.log记录<br><br>
