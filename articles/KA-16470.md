...
2. 将CRM数据库与Campaign客户同步需要设置一个流程，以便他们将其CRM数据推送到Campaign。 电子邮件CRM_ID是要同步的最低字段。

3. 在页面上使CRM_ID可用。这要求客户已登录。 登录后，CMS必须联系客户CRM以验证密码是否正确。 在此级别，还必须捕获唯一CRM_ID并将其发送到页面，最好是发送到数据层。

4. DTM会读取“CRM_ID配置位于Marketing Cloud ID服务”工具中。 在DTM中配置了hashed_email数据元素，以读取dataLayer中可用的CRM_ID。
...
为AAM创建的sFTP需要使用Adobe Campaign进行配置，以便Campaign可以从AAM推送和提取区段。 Adobe客户关怀团队或您的Adobe代表将向您提供sFTP凭据。 在配置期间，会配置ftp-in和ftp-out位置，营销活动使用这两个位置分别将受众发送到AAM，并将区段从AAM载入到营销活动。
...
6. &#x200B;&#x200B; &#x200B; &#x200B;使用AAM创建Datasources &amp; Destinations
...
7. &#x200B; &#x200B; &#x200B;在&#x200B;Adobe Campaign中配置数据源然后在Adobe Campaign中配置数据源。 将在Adobe Campaign UI中创建两个数据源；一个基于声明的ID或经过哈希处理的客户ID，另一个基于marketing cloud id或请求者UUID。 对应于每个ID的目标ID将添加到相应的字段中。 例如，AAM目标Adobe Campaign — 已声明ID的目标ID将添加到数据源设置中，并将协调键值作为经过哈希处理的客户ID。
...
8. 跟踪用户在网站上的行为在以下用例中，客户使用的搜索关键词会在evar2中捕获，而产品购买则作为Analytics中的购买事件。 以下流程基于AA调用到AAM的服务器端转发。 在搜索页面上，我们捕获“q”参数并将其放入数据元素中：
...
此时，我们能够跟踪客户并在Audience Manager中创建区段。 此外，我们已保证Audience Manager和Campaign都使用相同的CRM_ID收集用户。

9. &#x200B; &#x200B; &#x200B; &#x200B;根据用户行为在AAM中创建区段，然后创建以下特征：“已搜索冬季”：
...
10. &#x200B; &#x200B; &#x200B;将区段推送&#x200B;到Campaign要与Campaign共享AMC受众，必须在在AAM中创建区段后在Adobe Campaign中完成配置。 首先在Adobe Campaign中创建共享受众，然后从区段列表中选择AMC区段。 必须根据MID和营销活动中声明的ID创建两个共享受众。 完成配置后，区段会被视为自动映射到AAM中的各个目标。
...
11. 通过AMC与AAM共享促销活动受众要通过AMC与AAM共享促销活动受众，Adobe Campaign中需要以下配置。 在营销活动UI的“管理”菜单选项下，选择生产技术工作流。 单击新的技术工作流，拖动查询，然后更新共享受众，然后结束。
...
您可能尚未使用Adobe ID登录营销活动。 如果您使用Adobe ID登录，请确保已将您添加到Marketing CloudEnterprise Console UI中的受众库产品。
