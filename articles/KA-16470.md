...
# Audience Manager到 [!DNL Campaign]

## 描述


本技术实施指南提供了有关实施专家在将Audience Manager与集成时需要遵循的参考架构、准则、配置步骤和测试的信息 [!DNL Campaign].



本技术集成指南面向Adobe顾问、Adobe合作伙伴和其他经验丰富的实施专家。 配置此集成需要完全了解本指南中涵盖的每个解决方案的实施流程。
...
该集成利用了Marketing Cloud共享受众和AAM中声明的ID功能。
...
8. AdobeCampaignv6必须与Marketing-Cloud连接
9. 配置 [!DNL Campaign] Marketing Cloud
10. 用户应该能够登录到 [!DNL Campaign] V6通过Adobe ID
...
2. [!DNL Campaign] 配置需要由 [!DNL Campaign] 技术运营。
3. 数据源、AAM UI和AAM sFTP中的目标应通过您的Adobe代表进行创建/配置。
...
客户需要具有一个用于存储所有CRM数据的系统。 这必须是电子邮件地址列表，以及此客户的唯一哈希ID。 例如： *[abc@abcmail.com](mailto:abc@abcmail.com), 1231231j2g3123<b>[xyz@xyzmail.com](mailto:xyz@xyzmail.com), 234234jh23k4jh324* 其中，第一列是电子邮件地址，第二列是经过哈希处理的CRM_ID

2. 将CRM数据库与 [!DNL Campaign] 客户需要设置一个流程，以便将其CRM数据推送到 [!DNL Campaign]. 电子邮件CRM_ID是要同步的最低字段。

3. 在页面上使CRM_ID可用。这要求客户已登录。 登录后，CMS必须联系客户CRM以验证密码是否正确。 在此级别，还必须捕获唯一CRM_ID并将其发送到页面，最好是发送到数据层。

4. DTM会读取“CRM_ID配置”位于“Marketing CloudID服务”工具中。 在DTM中配置了hashed_email数据元素，以读取dataLayer中可用的CRM_ID。
...
为AAM创建的sFTP需要配置Adobe Campaign，以便 [!DNL Campaign] 可以从AAM推送和提取区段。 Adobe客户关怀团队或您的Adobe代表将向您提供sFTP凭据。 在配置期间，会同时配置FTP输入和FTP输出位置，这些位置由 [!DNL Campaign] 将受众发送到AAM并将区段从AAM载入到 [!DNL Campaign] 分别进行。
...
如果尚未创建，则应在AAM UI中创建集成的数据源和目标。 如果已创建此ID，则应使用数据源和目标ID在Adobe Campaign UI中配置它们。 应创建两个数据源，一个基于Marketing CloudID，另一个基于声明的ID。 目标在AAM Admin UI中创建，对应于每个数据源。 这需要AAM顾问或AAM Clientcare的参与。
...
7. 在Adobe Campaign中配置数据源随后在Adobe Campaign中配置数据源。 将在Adobe Campaign UI中创建两个数据源；一个基于声明的ID或经过哈希处理的客户ID，另一个基于Marketing CloudID或请求者UUID。 对应于每个ID的目标ID将添加到相应的字段中。 例如，AAM目标Adobe Campaign — 已声明ID的目标ID将添加到数据源设置中，并将协调键值作为经过哈希处理的客户ID。
...
8. 跟踪用户在网站上的行为在以下用例中，客户使用的搜索关键词会在evar2中捕获，产品购买作为 [!DNL Analytics]. 以下流程基于AA调用到AAM的服务器端转发。 在搜索页面上，我们捕获“q”参数并将其放入数据元素中：
...
然后，我们将其添加到页面加载规则中，在该规则中，我们将搜索关键词添加到evar2中：
...
此时，我们能够跟踪客户并在Audience Manager中创建区段。 此外，我们已保证Audience Manager和 [!DNL Campaign] 正在收集具有相同CRM_ID的用户。

9. 在AAM中根据用户行为创建区段，然后创建以下特征：“已搜索冬季”：
...
10. 将区段推送到 [!DNL Campaign] 与共享AMC受众 [!DNL Campaign]，则在AAM中创建区段后，必须在Adobe Campaign中完成配置。 首先在Adobe Campaign中创建共享受众，然后从区段列表中选择AMC区段。 必须根据MID和 [!DNL Campaign]. 完成配置后，区段会被视为自动映射到AAM中的各个目标。
...
11. 共享 [!DNL Campaign] 通过AAM共享受众 [!DNL Campaign] 通过AAM的受众，Adobe Campaign中需要以下配置。 在 [!UICONTROL 管理] 菜单选项 [!DNL Campaign] 用户界面中，选择生产技术工作流。 单击新的技术工作流，拖动查询，然后更新共享受众，然后结束。
...
创建后，这些区段会自动显示在AAM UI的“所有特征”下方的“Adobe Campaign特征”下
...
在Adobe Campaign中运行工作流，以将区段发送到AMC。 完成审核后，请检查审核。
...
在 [!DNL Campaign]，首先根据受众创建列表。 这位于“lists”中：
...
现在，您可以通过弹出窗口选择Marketing Cloud受众。 选择“search_for_winter_did_not-buy”：
...
在 [!DNL Campaign] 工作流中，您现在将能够选择此列表并发送定向电子邮件：
...
建议使用AAM中的测试区段运行示例工作流，以检查集成是否端到端工作。
...
以下设置将分别用于声明的ID和MID数据源。 无法将集成代码更改为任何其他值，否则客户可能会产生额外成本。
...
- 列表下没有“创建共享受众”选项，原因是什么？


您可能尚未使用Adobe ID登录到 [!DNL Campaign]. 如果您使用Adobe ID登录，请确保已将您添加到Marketing CloudEnterprise Console UI中的受众库产品。

- 我已与AAM共享区段，但在AAM UI中尚未看到该区段。 原因是什么？


运行工作流后，可能需要12-24小时才能将数据摄取到AAM中，另需24-48小时才能在AAM UI中更新报表。 如果在AAM UI中的特征/区段下即使在3天后仍未看到数字，请与AAM顾问联系。 </b>                                   <b>



</b>最佳实践<b>

1. 建议将AAM和 [!DNL Campaign] 顾问们通力协作，在两端完成设置。
2. 建议使用记录有限的区段来端到端测试集成。
3. 切勿将区段映射到 [!DNL Campaign] 目标
