...
使用者在重複造訪您的網站時遇到的第一個快取層級是他們自己的瀏覽器。  瀏覽器層級的快取通常透過 [<b>Cache-Control： max-age=...</b>](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Cache-Control) 回應 [頁首](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers).  此 <b>max-age</b> 設定可告知瀏覽器，在嘗試「重新驗證」或再次從網站要求檔案之前，應該快取檔案的秒數。  這個快取概念 <b>max-age</b> 通常稱為「快取有效期」或TTL （「存留時間」）。
...
現代瀏覽器全都支援 <b>Cache-Control</b> 但是，標頭中HTTP/1.0的某些舊式過時標頭仍可能會對快取產生影響。  這些標題為 <b>[過期](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Expires)</b> 和 <b>[Pragma](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Pragma)</b>.  如果您不需要支援非常舊的瀏覽器，那麼請勿傳送這些回應標頭。
除了快取以外，重新驗證也是一個重要的概念。  重新驗證仰賴 <b>[上次修改時間](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Last-Modified) </b>([回應](https://developer.mozilla.org/en-US/docs/Glossary/Response_header)) /<b> [If-Modified-Since](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/If-Modified-Since)</b> ([請求](https://developer.mozilla.org/en-US/docs/Glossary/Request_header))標頭，以及 <b>[ETag](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/ETag)</b> （回應） / <b>[If-None-Match](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/If-None-Match)</b> （請求）標頭。
...
CDN會依照類似瀏覽器的規則快取內容。  他們仰賴 <b>[Cache-Control](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Cache-Control) </b>HTTP [回應](https://developer.mozilla.org/en-US/docs/Glossary/Response_header) 頁首，通常回退到 <b>[過期](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Expires)</b> 頁首（若否） <b>Cache-Control</b> 找到標頭。
...
- Microsoft IIS — 使用 [動態壓縮](https://docs.microsoft.com/zh-cn/iis/configuration/system.webserver/httpcompression/).
- 不允許大型檔案或已經壓縮的檔案使用gzip壓縮。  請注意，大部分的影像和視訊格式都已預先壓縮。  在Web伺服器層級即時壓縮資料夾，需要非常高的效能成本。
   ...
   - 在IIS上，這可以透過 [`<` dynamicType`>`  設定](https://docs.microsoft.com/zh-cn/iis/configuration/system.webserver/httpcompression/).
...
- [CDN上限](https://www.stackpath.com/maxcdn/)
...
1. 設定下列HTTP回應標題，AEM未依預設設定這些標題。
   ...
   1. <b>[Cache-Control： max-age=...](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Cache-Control) </b> — 若要設定此標頭， [ACS Commons - Dispatcher TTL](https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-ttl/index.html) 可使用，或實作自訂程式碼以進行設定。
...
2. 如果網站有個人化/動態內容：
   ...
   1. 使用 [Apache Sling Dynamic Include](https://helpx.adobe.com/cn/experience-manager/kt/platform-repository/using/sling-dynamic-include-technical-video-setup.html) 將內容分組，以便針對不同的時段快取頁面的不同部分。
...
3. 最佳化使用者端程式庫。
   ...
   1. [啟用縮制](https://helpx.adobe.com/cn/experience-manager/6-3/sites/developing/using/clientlibs.html) 在使用者端資料庫上。
   ...
   3. [內嵌使用者端](https://helpx.adobe.com/cn/experience-manager/6-3/sites/developing/using/clientlibs.html)[資料庫](https://helpx.adobe.com/cn/experience-manager/6-3/sites/developing/using/clientlibs.html) 以精簡及減少js和css檔案。
