...
1. &#x200B;<b>私人 </b>- <b>私人</b> 指令 <b>Cache-Control</b> 标头会使文件仅在浏览器中缓存，而不是在CDN等中间缓存中缓存。  如果您的页面包含特定于用户的个性化内容，则此指令的实际用途是。 
   ...
   <b>  </b>Cache-Control:max-age=300，私有
...
在Google Chrome中测试缓存时，如果您通过https进行测试，并且拥有自签名证书，则不会缓存任何内容。  [当存在不受信任或无效的证书时，Chrome不会缓存响应或执行重新验证](https://helpx.adobe.com/experience-manager/kb/cache-problems-on-chrome-with-SSL-certificate-errors.html).
...
2. 所有未预压缩的文件类型的GZip压缩响应。
   ...
   - 
      ...
      - Apache — 使用 [mod_deflate](https://httpd.apache.org/docs/2.4/mod/mod_deflate.html).  当mod_deflate使用Vary时要小心。  在某些情况下，Vary标头可能会导致CDN和浏览器完全跳过缓存。
...
3. 您的CDN提供商是否支持 [边缘侧包括](https://en.wikipedia.org/wiki/Edge_Side_Includes) (ESI)然后利用此功能。

   - 


      AEM组件可使用ESI进行划分。  要实现此目的，请使用Apache [Sling动态包含](https://github.com/Cognifide/Sling-Dynamic-Include) 或实施自定义解决方案。
   - 
...
<br><br><br><br> <br><br>AEM Dispatcher缓存<br><br><br><br> <br><br>
如果CDN缓存过期，则请求将到达AEM调度程序缓存。  在此级别，可以执行许多操作来优化缓存。

由于这是一个较大的主题，请参阅 [本文](https://helpx.adobe.com/experience-manager/kb/optimizing-the-dispatcher-cache.html) 有关如何优化调度程序缓存的详细信息。
 <br><br>AEM发布实例<br><br><br><br> <br><br>在AEM级别，应执行以下几项操作来优化各种缓存层：
1. 设置以下HTTP响应标头，默认情况下，AEM未设置这些标头。
   1. <b>[Cache-Control:max-age=...](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Cache-Control) </b> — 要设置此标题， [ACS Commons — 调度程序TTL](https://adobe-consulting-services.github.io/acs-aem-commons/features/dispatcher-ttl/index.html) 或实施自定义代码以对其进行设置。
...
2. 如果网站具有个性化/动态内容：
   ...
   1. 使用 [Apache Sling Dynamic Include](https://helpx.adobe.com/experience-manager/kt/platform-repository/using/sling-dynamic-include-technical-video-setup.html) 来划分内容，以便可以在不同时间段缓存页面的不同部分。
...
3. 优化客户端库。
   ...
   3. [嵌入客户端](https://helpx.adobe.com/experience-manager/6-3/sites/developing/using/clientlibs.html)[库](https://helpx.adobe.com/experience-manager/6-3/sites/developing/using/clientlibs.html) 缩小和减少js和css文件。
