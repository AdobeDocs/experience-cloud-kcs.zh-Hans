...
我们的一位AEMaaCS客户会定期阻止复制队列。 本文介绍了工程团队建议的解决方法，该解决方法可以使用管理员权限在UI中执行。
...
由于javax.jcr.NamespaceException：未知的命名空间前缀：ns22_异常，我们的某个AEMaaCS客户复制队列定期被阻止。
...
取消阻止队列只需从golden-publish分发队列UI中删除请求即可。 建议客户执行以下步骤。
