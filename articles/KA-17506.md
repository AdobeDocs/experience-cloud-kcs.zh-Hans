...
大多数高吞吐量短消息服务中心(SMS-C)与SMPP协议版本3.4兼容。该协议允许发送短信并接收有关这些短信投放的信息。 SMPP 协议以 PDF 文档形式记录在互联网上的 SMPP 协议规范 v3.4 中。
...
<b>在没有Wireshark的情况下捕获网络流量</b>

如果您没有直接访问计算机的权限，则可能需要使用命令行工具（如tcpdump）进行捕获。 如果您已经知道连接的 TCP 端口，请放置正确的过滤器以避免捕获所有流量。 以下是捕获端口12345到 <b>outfile.pcap</b>:
...
- 在SMS-C端口上使用显示过滤器。<br><br>    例如，如果SMS-C使用端口10000，请使用以下过滤器：
   `tcp.port == 10000`
...
- 使用 <b>跟踪TCP流</b> 工具来隔离您正在处理的流。<br><br>关闭弹出的红色/蓝色文本窗口，因为它只对与 SMPP 无关的文本协议有用。
...
注意：标准命令列表可在SMPP规范（SMPP命令集）的5.1.2.1节中找到。
