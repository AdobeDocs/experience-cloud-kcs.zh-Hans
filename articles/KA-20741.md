...
我们的一位客户抱怨其Stage发布程序无法正常运行，并且无法在此实例上发布内容，并且在error.log中可能会看到以下错误：
...
日志中的错误与缓存无关，但当服务用户对Author或Publisher上的/var/discovery路径没有正确的权限时，就会出现这些错误。

在进一步调查时，在要求客户验证/var/discovery路径的权限时，发现在Stage发布者上/var下没有“discovery”文件夹，并且还有大量缺失的内容。

虽然生产发布者上的/var文件夹有20多个子文件夹，但Stage发布者上的/var文件夹只有2个 — “replication”和“sling”。
