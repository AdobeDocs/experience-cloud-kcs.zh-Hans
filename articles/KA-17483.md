...
[&lt; — 上一个：常用日志](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17914.html)
...
![展示了一个保龄球道，球在车道上滚动。  球有一个箭头，上面有一个字。  排水沟阻塞器被抬起，上面有不可改变的文字。](assets/bowling-file-immutability.png "bowling-file-immunitable")
...
现在，Apache Webserver知道域是它应该产生流量的域，但我们仍需要通知调度程序模块它有一个新的域名来遵守。  我们将首先创建一个 `*_vhost.any` 文件 `/etc/httpd/conf.dispatcher.d/vhosts/weretail_vhosts.any` 在文件中，我们将输入我们要遵循的域名：
