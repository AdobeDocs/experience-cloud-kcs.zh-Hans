...
<br><br><br> <br><br>
当AMS设置系统时，它们会推出基线配置，使所有功能都能够正常运行并且安全。  AMS不希望将这些内容作为功能和安全性的基准。  要完成此操作，某些文件会被标记为只读和不可变，以避免您更改它们。
...
现在，Apache Webserver知道域是某些内容应该会产生流量，但我们仍需要通知调度程序模块，它有一个新的域名可供遵循。  我们首先将创建一个新的\*_vhost.any文件/etc/httpd/conf.dispatcher.d/vhosts/weretail_vhosts.any ，然后在该文件中放置我们要遵循的域名：
