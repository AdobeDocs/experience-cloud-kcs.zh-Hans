...
<b>问题/症状</b><br>此 <b>持续集成</b> 和 <b>连续部署</b> <b>(CI/CD)</b> 管道在 *部署到开发* 暂存。 <br>由于代码库，部署日志未显示任何错误，但存在以下错误：<br> <br>`[` 1`]`  
...
这意味着复制队列被阻止，从而妨碍了其他可变内容的安装。

在日志中，错误 <b>`[` 1`]` </b> 在部署时间范围内存在。 最可能的原因是节点 —  <b>`[` 2`]` </b> 在发布服务器和/或预览服务器上不存在，因此复制失败。
