---
title: 「管道失敗並出現錯誤\「缺少end_aem_createsegmentstore屬性\」
description: 描述
...
# 管道失敗，並出現錯誤「缺少end_aem_createsegmentstore屬性」

## 描述 {#description}

在「部署至開發」階段期間，CI/CD管道持續失敗。 <br><br>由於程式碼基底，部署記錄檔未顯示任何錯誤，但出現以下錯誤：<br><br> <br><br> <br><br> <br><br>`[` 1`]`  
...
這表示會封鎖復寫佇列，阻礙其他可變內容的安裝。

在記錄中，錯誤 `[` 1`]`  在部署時間範圍內存在。 很可能是節點 —  `[` 2`]`  發行者和/或預覽伺服器上不存在，因此復寫失敗。
