---
title: “从JVM进行线程转储”
description: “了解如何跨各种操作系统从JVM进行线程转储。”
...
线程转储是Java虚拟机(JVM)中当前处于活动状态的所有Java线程的列表。 本文讨论了从JVM进行线程转储的几种方法。
...
- 在Linux和UNIX中，您需要以拥有java进程的用户身份运行命令：<br>\
...
- 如果java进程没有响应，则使用选项有时可能会有所帮助 <b>-J-d64</b> （在64位系统中），例如：<br>\
  `jstack -J-d64 -l   <pid>   >> threaddumps.log`
- 如果jstack命令(`jstack -l <pid> >> threaddumps.log`)引发错误 `[` 1`]`  下面，以拥有java进程的用户身份运行命令。 例如：<br>\
