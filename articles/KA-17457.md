...
2. 如果AEM java进程导致CPU利用率较高，请在操作系统级别检查：       如果AEM导致CPU利用率较高，请运行现成的分析工具几分钟并分析结果。
...
4. 查看系统维护过程。 请参阅 [文章](https://experienceleague.adobe.com/docs/experience-manager-65/deploying/configuring/monitoring-and-maintaining.html) 有关AEM维护的详细信息，并确保您对AEM进行正确维护，包括：
...
- 调整资产处理性能，请参阅 [本文](https://experienceleague.adobe.com/docs/experience-manager-65/assets/administer/performance-tuning-guidelines.html).
...
1. 在AEM实例上打开这些URL以查看有关异步索引器的统计资料：http://aemhost:port/system/console/jmx/org.apache.jackrabbit.oak%3Aname%3Dasync%2Ctype%3DIndexStats http://aemhost:port/system/console/jmx/org.apache.jackrabbit.oak%3Aname%3Dfulltext-async%2Ctype%3DIndexStats  - This URL仅适用于AEM6.2及更高版本
2. 在每个页面上，检查以下字段：
...
2. 如果问题是由于工作流运行不有效，您可以检查并发 [工作流处理提示](https://experienceleague.adobe.com/docs/experience-manager-65/developing/extending-aem/extending-workflows/workflows-process-ref.html?lang=en).
