...
4. 对步骤2中找到的所有节点重复步骤3
   ...
   上面这个rmNode命令应该为损坏的路径返回true ，这意味着它删除了此损坏路径。 通过在这些路径上重新运行rmNode命令，确保删除这些找到的三个损坏的路径。 下次运行应该返回 false。如果您发现存储库中仍然存在相同的路径，请使用 <b>oak-run jar的补丁版本，即oak-run-1.2.18-NPR-17596</b>. 此版本的jar在压缩时跳过不可读的二进制文件，将它们替换为0字节二进制文件并记录异常和syserr的路径。 随后，生成的压缩存储库应该会通过oak-run检查和节点计数脚本，您还应该能够使用未打补丁的oak-run再次压缩它。
