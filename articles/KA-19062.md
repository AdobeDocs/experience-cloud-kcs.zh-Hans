...
原因<br>
错误日志中的SegmentNotFoundException表示区段不再存在，尽管显然有人尝试仍访问该区段。 这大致有三种不同的根本原因：区段已通过手动干预删除（例如rm -rf /），区段已通过修订垃圾收藏集删除，或者由于代码中的一些错误无法找到该区段。
<br>要解决的步骤<br>
有两种方法可以修复问题并删除存储库中的不一致内容

<b>还原到区段存储的最后一个已知良好修订。</b>

首先，使用oak run工具，它是可运行的jar1，包含简单Oak安装和执行与oak相关的操作所需的一切。

oak-run的检查运行模式可用于确定区段存储的最后一个已知良好修订。  这可用于手动将损坏的区段存储恢复到其最新的良好修订。
...
3. 运行一致性检查，java -Xmx6000m -jar oak-run-\*.jar check —bin=-1 /path/to/crx-quickstart/repository/segmentstore这会向后搜索修订，直到找到一致的修订：查找如下消息：
...
*在Oak运行检查无法找到良好修订的情况下，我们会在运行检查命令时获得“ConsistencyChecker — 未找到良好修订”。*
<b>如何修复一致性检查中遇到“ConsistencyChecker — 未找到良好修订”时的损坏</b>
<b>手动删除损坏的节点。</b>

可以在AEM、未配置FileDatastore的TarMK设置以及二进制文件中有损坏的情况下执行以下操作。
...
在某些情况下，该问题与二进制属性有关，并且childCount groovy脚本无法找到任何损坏的节点。  在这些情况下，您可以改用以下命令，它将读取遍历期间遇到的每个二进制文件的前1024个字节（请注意，此命令会较慢，并且仅当上述命令未返回预期结果时才应使用）：
...
1. 使用rmNodes.groovy删除最后一个命令输出中列出的所有已识别的损坏节点


使用以下命令加载oak-run控制台外壳：
...
其中损坏的节点路径为步骤2中获得的路径，例如：“/content/dam/test.txt/jcr：content/renditions/original/jcr：content/”注意：使用oak-run.jar版本1.6.13及更高版本时，请设置 — read-write JVM参数，如果您遇到错误，例如：
...
上面这个rmNode命令应该为损坏的路径返回true ，这意味着它删除了此损坏路径。 通过在这些路径上重新运行rmNode命令，确保删除这些找到的三个损坏的路径。 下次运行应该返回 false。

如果您发现存储库中仍然存在相同的路径，请使用 <b>oak-run jar的补丁版本，即oak-run-1.2.18-NPR-17596</b>. 此版本的jar在压缩时跳过不可读的二进制文件，将它们替换为0字节二进制文件并记录异常和syserr的路径。 随后，生成的压缩存储库应该会通过oak-run检查和节点计数脚本，您还应该能够使用未打补丁的oak-run再次压缩它。

1. 通过使用以下列出检查点来执行检查点清理。 如果有多个检查点，则清理它们：

   ```
   nohup java -Xmx4096m -jar oak-run-1.2.18.jar checkpoints /app/AEM6/author/crx-quickstart/repository/segmentstore rm-allnohup.out &
   ```
2. 运行离线压缩。 如果您不知道如何运行离线压缩，请参阅 [此处](https://gist.github.com/andrewmkhoury/0b1fe4d8b619178ff87b).
3. 启动服务器等待索引完成。
