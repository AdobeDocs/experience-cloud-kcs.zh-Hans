...
# “formAlreadyMessage”和“datatypeMessage”消息未从英语翻译
...
客户报告的消息未国际化。 错误消息“Invalid date - value must be of date datatype”（无效日期 — 值必须为date数据类型），即使应使用德语（客户的浏览器语言）也是如此。

检查德语clientlib /libs/fd/af/runtime/clientlibs/I18N/de/javascript/LogMessages.js时，我们发现消息为英语，并且进一步添加了占位符{0}，这意味着消息的一部分是动态的。

通过检查替换占位符(EnglishMessage03.JPG)的函数，我们可以看到，“date”值是硬编码的，并且是英语的，很难翻译。

<u>重现问题的步骤：</u>

- 检查德语或除英语clientlib之外的任何其他语言/libs/fd/af/runtime/clientlibs/I18N/de/javascript/LogMessages.js
- 请参阅“formAlreadyMessage”和“datatypeMessage”消息中的英文。
