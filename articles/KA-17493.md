...
[&lt; — 上一个：使用和了解变量](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17487.html)
...
![创作激活内容，这会触发发布者向调度程序发送刷新请求](assets/dispatcher-flushing-order-of-events.png "dispatcher-flushing-order-of-events")
...
![AEM网页/etc/replication.html中标准复制代理的屏幕截图](assets/author-rep-agent-example.png "author-rep-agent-example")
...
![AEM网页/etc/replication.html中标准刷新复制代理的屏幕截图](assets/publish-flush-rep-agent-example.png "publish-flush-rep-agent-example")

### 调度程序刷新复制接收虚拟主机

调度程序模块会查找特定的标头，以便知道何时POST请求是要传递到AEM呈现器的内容，或者它是否序列化为刷新请求，并且需要由调度程序处理程序本身处理。

以下是配置页面的屏幕截图，其中显示了这些值：
![“序列化类型”显示为“调度程序刷新”的主配置屏幕设置选项卡的图片](assets/disp-flush-agent1.png "disp-flush-agent1")

默认设置页面显示 `Serialization Type` as `Dispatcher Flush` 并设置错误级别

![复制代理的传输选项卡的屏幕截图。  这会显示将刷新请求发布到的URI。  /dispatcher/invalidate.cache](assets/disp-flush-agent2.png "disp-flush-agent2")

在 `Transport` 选项卡 `URI` 设置为指向将接收刷新请求的调度程序的IP地址。  路径 `/dispatcher/invalidate.cache` 不是模块确定是否是刷新的方式，它只是您在访问日志中可以看到的一个明显端点，以知道它是刷新请求。  在 `Extended` 选项卡，我们将检查其中的内容以确定这是对调度程序模块的刷新请求。

![复制代理的“扩展”选项卡的屏幕截图。  请注意随发送的POST请求一起发送的标头，以告知调度程序刷新](assets/disp-flush-agent3.png "disp-flush-agent3")
...
![标准复制代理的屏幕，显示当从创作发布内容的复制事件中收到新项目时，具有反应和触发功能的复制代理](assets/disp-flush-agent4.png "disp-flush-agent4")
...
![与显示的stat级别的内容和日期混合的stat文件](assets/dispatcher-statfiles.png "dispatcher-statfiles")
...
[下一个 — >虚URL](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17463.html)