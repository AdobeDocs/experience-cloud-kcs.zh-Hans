...
<br><br><br><br>工作原理<br><br><br><br><br><br>操作顺序<br><br><br><br><br><br>
...
<br><br><br><br><br><br>创作复制代理<br><br><br><br><br><br>
...
<br><br><br><br><br><br>发布者复制代理<br><br><br><br><br><br>
以下是已配置标准刷新复制代理的屏幕截图示例
![AEM网页/etc/replication.html中标准刷新复制代理的屏幕截图](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/publish-flush-rep-agent-example.png "publish-flush-rep-agent-example")<br><br><br><br><br><br>调度程序刷新复制接收虚拟主机<br><br><br><br><br><br>
...
默认设置页面显示 <b>[!UICONTROL 序列化类型]</b> as *[!UICONTROL 调度程序刷新]* 并设置错误级别
...
在 [!UICONTROL 运输] 选项卡 [!UICONTROL URI] 设置为指向将接收刷新请求的调度程序的IP地址。  路径 `/dispatcher/invalidate.cache` 不是模块确定是否是刷新的方式，它只是您在访问日志中可以看到的一个明显端点，以知道它是刷新请求。  在 [!UICONTROL 扩展] 选项卡，我们将检查其中的内容以确定这是对调度程序模块的刷新请求。
...
的 [!UICONTROL HTTP方法] 对于刷新请求，只是具有一些特殊请求标头的GET请求：
...
- Host
   - 这是 `Host` 标头会被欺骗以定位特定 `VirtualHost` 在调度程序上配置的 [!DNL Apache] Web服务器(`/etc/httpd/conf.d/enabled_vhosts/aem_flush.vhost`)。  硬编码值与 `aem_flush.vhost` 文件 *`ServerName`* 或 *`ServerAlias`*
...
在 [!UICONTROL 触发器] 选项卡，我们将注意我们使用的切换触发器及其含义
...
![与显示的stat级别的内容和日期混合的stat文件](https://helpx.adobe.com/content/dam/help/en/experience-manager/kb/ams-dispatcher-manual/disp-flushing/jcr_content/main-pars/image_2035486865/dispatcher-statfiles.png "dispatcher-statfiles")<br><br><br><br><br><br>统计文件级别<br><br><br><br>
请注意，在每个目录中， `.stat` 文件存在。  这表示已发生刷新。  在上面的示例中， <b>`statfilelevel`</b> 将设置为 <b>`3`</b> 在相应的场配置文件内。

的 `statfilelevel` 设置指示模块将遍历和更新的文件夹深度 `.stat` 文件。  .stat文件为空，它只是带有日期戳的文件名，甚至可以手动创建，但在调度程序服务器的命令行中运行touch命令。
...
<br><br><br><br> <br><br>STAT文件时间戳握手<br><br><br><br><br><br>
当内容请求进入同一例程时
...
<br><br><br><br><br><br>缓存握手 — 示例1<br><br><br><br><br><br>
...
<br><br><br><br><br><br>缓存握手 — 示例2<br><br><br><br><br><br>
...
<br><br><br><br><br><br>场文件设置<br><br><br><br><br><br>
...
此目录应与 [!DNL Apache] web服务器配置为使用的域的文档根设置。

在每个存放子文件夹的场中具有嵌套的docroot文件夹 [!DNL Apache] 文档根是一个糟糕的主意，原因有很多。


<br><br><br><br><br><br>统计文件级别<br><br><br><br><br><br>
此配置条目位于场文件的以下部分：
<br><br><br><br><br> <br><br><br><br>
...
<br><br><br><br><br><br>允许失效<br><br><br><br><br><br>
此配置条目位于场文件的以下部分：
<br><br><br><br><br> <br><br><br><br>
...
<br><br><br><br><br><br><br>失效规则<br><br><br><br><br><br>
此配置条目位于场文件的以下部分：
<br><br><br><br><br> <br><br><br><br>
...
<br><br>测试/疑难解答<br><br><br><br><br><br>
...
向调度程序发出request命令后，您将需要查看它在日志中的操作以及它在 `.stat files`.  跟踪日志文件，您应会看到以下条目，以确认刷新请求已点击调度程序模块
<br><br><br><br><br> <br><br><br><br>
