...
![作者激活内容，这会触发发布者向调度程序发送刷新请求](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/dispatcher-flushing-order-of-events.png "dispatcher-flushing-order-of-events")
...
当发布者收到文件时，会将复制代理配置为指向接收事件触发的调度程序。  然后，它将序列化刷新请求并将其发布到调度程序。
<br>创作复制代理<br>
以下是配置的标准复制代理屏幕截图示例
![AEM网页/etc/replication.html中的标准复制代理屏幕截图](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/author-rep-agent-example.png "author-rep-agent-example")
...
![AEM网页/etc/replication.html中的标准刷新复制代理屏幕截图](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/publish-flush-rep-agent-example.png "publish-flush-rep-agent-example")<br>DISPATCHER刷新复制接收虚拟主机<br>
调度程序模块会查找特定的标头，以了解POST请求何时可以传递给AEM渲染，或者是否将其序列化为刷新请求，以及是否需要由调度程序处理程序本身进行处理。  以下是显示这些值的配置页面屏幕截图：
![序列化类型显示为“调度程序刷新”的主配置屏幕设置选项卡图片](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/disp-flush-agent1.png "disp-flush-agent1")
默认设置页面显示 <b>序列化类型</b> 作为 *Dispatcher刷新* 并设置错误级别
![复制代理的传输选项卡屏幕截图。  该图显示了将刷新请求发布到的URI。  /dispatcher/invalidate.cache](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/disp-flush-agent2.png "disp-flush-agent2")
...
![复制代理的扩展选项卡屏幕截图。  请注意与发送的POST请求一起发送以告知调度程序刷新的标头](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/disp-flush-agent3.png "disp-flush-agent3")
...
- Host
   - 这是欺骗Host Header以定位在Dispatcher Apache Web Server上配置的特定VirtualHost的位置(/etc/httpd/conf.d/enabled_vhosts/aem_flush.vhost)。  这是与aem_flush.vhost文件中的条目匹配的编码值。 *服务器名称* 或 *ServerAlias*

![标准复制代理屏幕，显示当从作者发布内容中收到来自复制事件的新项目时，复制代理会作出反应并触发](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/disp-flush-agent4.png "disp-flush-agent4")
...
![混合了内容和日期的statfiles，显示了stat级别](https://helpx.adobe.com/content/dam/help/en/experience-manager/kb/ams-dispatcher-manual/disp-flushing/jcr_content/main-pars/image_2035486865/dispatcher-statfiles.png "dispatcher-statfiles")<br>stat文件级别<br>
请注意，每个目录中都有一个.stat文件。  这是一个表明已发生刷新的指示器。  在上面的示例中， <b>statfilelevel</b> 设置已设置为 <b>3</b> 在相应的场配置文件中。

statfilelevel设置表明模块将遍历并更新.stat文件的文件夹层数。  .stat文件为空，它只不过是带有datestamp的文件名，甚至可以手动创建，但可以在调度程序服务器的命令行上运行touch命令。
