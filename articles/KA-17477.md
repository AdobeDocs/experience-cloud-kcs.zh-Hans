...
![apache基线包含顺序。 apache二进制文件以httpd.conf开头，该属性对conf.d/起到了includeoptional作用。*.conf和conf.modules.d/*.conf目录](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/Apache-Webserver-Baseline-Includes.png)
...
![AMS基线包括以dispatcher_vhost.conf开头，将包含/etc/httpd/conf.d/enabled_vhosts/目录中任何带有*.vhost的文件。 /etc/httpd/conf.d/enabled_vhosts/目录中的项目是位于/etc/httpd/conf.d/available_vhosts/中的实际配置文件的符号链接](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/Apache-Webserver-AMS-Baseline-Includes.png)
...
![ams虚拟主机包含子项。 此图片显示了一个.vhost文件如何包含来自变量、白名单和重写文件夹的文件](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/Apache-Webserver-AMS-Vhost-Includes.png)
...
![<FILENAME>_farms.any将包含sub .any文件以完成场配置。 在此图片中，您可以看到场将包含每个顶级节文件缓存、clientheader、过滤器、renders和vhosts .any文件](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/Apache-Webserver-AMS-Farm-Includes.png)
