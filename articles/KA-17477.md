...
| FILENAME.vhost | 暂存： /etc/httpd/conf.d/available_vhosts/<br><br>活动：<br><br>/etc/httpd/conf.d<br><br><u>*注意：.vhost文件不会复制到enabled_vhosts文件夹中，而是使用符号链接指向available_vhosts/ .vhost文件的相对路径</u><br><br> | \*.vhost（虚拟主机）文件是VirtualHosts条目，用于匹配主机名，并允许Apache使用不同的规则处理每个域流量。 从.vhost文件中，将包含重写、白名单等其他文件。 |
...
<br><br><br><br> <br><br>Apache基线包含<br><br><br><br><br><br>![apache基线包含顺序。  apache二进制文件以httpd.conf开头，该文件对conf.d/执行includeoptional *.conf和conf.modules.d/*.conf目录](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/Apache-Webserver-Baseline-Includes.png "Apache-Webserver-Baseline-Include")<br><br><br><br> <br><br>
...
| 1<br>  2<br>  3<br>  4<br>  5<br>  6<br>  7<br>  8<br>  9<br>  10<br>  11<br>  12<br>  13<br>  14<br>  15<br>  16<br>  17<br>  18 | “Include``/etc/httpd/conf``.d``/variables/weretail``.vars``VirtualHost *:80``    ``ServerName   ``&quot;${MAIN_DOMAIN}&quot;``    ``目录/``        ``包括``/etc/httpd/conf``.d``/whitelists/weretail``*_whitelist.rules``        ``IfModule disp_apache2.c``            ``....史奈普…….``            ``SetHandler Dispatcher-handler``        ````/IfModule````        ``....史奈普…….``    ````/Directory````    ``....史奈普…….``    ``IfModule mod_rewrite.c``        ``ReWriteEngine on``        ``LogLevel警告rewrite:trace1``        ``包括``/etc/httpd/conf``.d``/rewrites/weretail_rewrite``.rules&quot; &quot;&quot;/IfModule&quot;&quot;&quot;/VirtualHost&quot;` |
| --- | --- |
...
接下来，让我们看一下 <b>/etc/httpd/conf.dispatcher.d/filters/weretail_search_filters.any</b>:
<br><br><br><br><br> <br><br><br><br>
