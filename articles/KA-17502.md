...
[&lt; — 上一个：什么是“调度程序”](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17911.html)
...
此目录包含 `_cache.any`, `_invalidate.any` 您创建的文件包含您希望模块如何处理来自AEM的缓存元素以及失效规则语法的规则。  有关此部分的更多详细信息为 [此处](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html?lang=en#configuring-the-dispatcher-cache-cache)
...
例如，在更新安装期间， RPM将查看 `httpd.conf` 如果它在 `unaltered` 声明 *替换* 文件，你就会得到重要的更新。  如果 `httpd.conf` was `altered` 然后 *不会替换* 该文件将创建一个名为 `httpd.conf.rpmnew` 而且，在服务启动时不适用的文件中，将会提供非常需要的修复。
...
[下一步 — >配置文件说明](https://experienceleague.adobe.com/docs/experience-cloud-kcs/kbarticles/KA-17477.html)