...
在AMS中，基本安装使用Enterprise Linux作为基本操作系统。 安装Apache Webserver时，它将具有默认安装文件集。 以下是通过安装提供的基本RPM而安装的默认文件 `yum` 存储库。
...
请记住，旧目录和新目录将映射回原始装载点，以消除混淆。 使用单独的卷不是必需的，但值得注意的是
...
- `/etc/httpd/conf.d/available_vhosts/`
   - 此文件夹将托管您的所有VirtualHost /文件，这些文件名为 `.vhost`
...
1. <b>重写目录</b>    此目录可以包含所有 `\*_rewrite.rules` 您创建的包含您的典型 *重写规则* 与Apache Web Server接触的语法 [mod_rewrite](https://httpd.apache.org/docs/current/mod/mod_rewrite.html) 模块。
...
- `/etc/httpd/conf.dispatcher.d/available_farms/`
   - 此文件夹将托管您的所有 `/myfarm {` 已调用的文件 `*_farm.any`
...
还有一些片段是Dispatcher场文件配置的子部分，我们创建了子目录，以便采用简洁的方式分隔这些文件，而不是将所有文件放在一个目录中。

1. <b>缓存目录</b>    此目录包含所有 `*_cache.any`， `*_invalidate.any` 您创建的文件，其中包含您希望模块如何处理来自AEM的缓存元素以及失效规则语法的规则。  有关本部分的更多详细信息为 [此处](https://docs.adobe.com/content/help/zh-Hans/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html#configuring-the-dispatcher-cache-cache)
...
3. <b>筛选器目录</b>    此目录可以包含所有 `*_filters.any` 您创建的包含所有筛选规则的文件，用于阻止或允许流量通过调度程序到达AEM。

   - `/etc/httpd/conf.dispatcher.d/filters/`
4. <b>渲染目录</b>    此目录可以包含所有 `*_renders.any` 您创建的文件，其中包含到Dispatcher将从中使用内容的每个后端服务器的连接详细信息。
...
请注意，我们未修改Apache提供的任何现有文件。 我们刚刚将我们的目录添加到他们将要使用的目录中。

现在，我们在文件中使用了模块 `/etc/httpd/conf.d/dispatcher_vhost.conf` 它会初始化我们的模块并加载特定于初始模块的配置文件。
...
您会再次注意到我们添加了文件和模块，但没有更改任何原始文件。  这为我们提供了所需的功能，并保护我们免受缺少所需的修补程序修复以及保持与包每次升级的最高兼容性级别的影响。
