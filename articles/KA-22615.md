...
客户正在开发一个网站，该网站应使用AEM Headless解决方案从其AEM实例中检索数据。

要设置端点，请参阅中的教程 `[` 1`]`  被跟踪了。

发布所有内容片段后，所有模型、内容、引用、资源和查询，如中所述 `[` 2`]` ，则在尝试从发布实例检索任何内容片段时，收到404错误，导致无法从发布者中读取内容。
...
`[` 1`]`  aem_base_version： 6.5 aem_envId：xxxxxxx aem_envType：dev aem_program_id：xxxxxx aem_release_id：xxxxx aem_service：cm-pxxxxxx-exxxxxxx aem_tenant：ns-team-aem-cm-prd-nxxxxxx aem_tier：发布后端：publishfarm/- cache： <b>已阻止</b>
集群： `<` 集群`>`
代码： - file_path： /var/log/aem/dispatcher.log host： publish-pxxxxxx-exxxxxxx.adobeaemcloud.com方法：POST命名空间：ns-team-aem-cm-prd-nxxxxxx orig_time：2023年7月25日:15:39:11 +0000路径： /content/cq：graphql/graphql/endpoint.json pod_name： `<` pod_name`>`
pod_uid： `<` pod_uid`>`
